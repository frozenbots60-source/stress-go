...
// ==========================================
var (
    SERVER_URL             = getEnv("TARGET_URL", "http://localhost:8080")
    VERSION                = "6.3.0"
    LOCALE                 = "en"
    PLATFORM               = "stake.com"
    TOTAL_CLIENTS          = 500
...
func (c *StressClient) Connect() bool {
    ...
    handshakeURL := fmt.Sprintf("%s/socket.io/?EIO=4&transport=polling", SERVER_URL)
...
func main() {
    ...
    <-done // This will block forever unless a signal is sent (simplified for this snippet)
